...
def batch_predrop(dataset, modality_config):
    modality_dropout = {
        modality_name: BatchDropout(config['padding'], config['dropout']) if config['dropout'] else None
        for modality_name, config in modality_config.items() if config['predrop']}

    def drop(batch):
        return {k: modality_dropout[k](v) if modality_dropout[k] else v for k, v in
                 batch.items()}  # Dropout

    return dataset.map(drop, batched=True)
